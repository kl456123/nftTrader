...
async function main() {
  ...
  const wyvernExchangeWithBulkCancellations =
    await WyvernExchangeWithBulkCancellations.deploy(
      registryAddress,
  ...
  console.log(
    'WyvernExchangeWithBulkCancellations deployed to:',
    wyvernExchangeWithBulkCancellations.address
  )
  const wyvernProxyRegistry = await ethers.getContractAt(
    'WyvernProxyRegistry',
    registryAddress
  )
  await wyvernProxyRegistry.grantInitialAuthentication(
    wyvernExchangeWithBulkCancellations.address
  )
}
